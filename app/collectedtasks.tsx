...
/**
 * CollectedTasks component displays tasks that have been sent and are pending collection.
 */
export default function CollectedTasks() {
  ...
  /**
   * Fetch all collected tasks from the database where task_status = 'sent'.
   */
  const fetchCollectedTasks = async () => {
    try {
      setLoading(true);
      ...
      console.error("Error fetching collected tasks:", err);
      setError("Failed to load collected tasks.");
    } finally {
      setLoading(false);
    }
  };
  ...
  /**
   * Mark a task as collected by updating task_status to 'collected'.
   ...
   */
  const markTaskAsCollected = async (
    taskType: string,
    ...
    try {
      ...
      Alert.alert("Success", "Task marked as collected.");
      fetchCollectedTasks();
    } catch (err) {
      console.error(`Error marking ${taskType} as collected:`, err);
      Alert.alert("Error", `Failed to mark ${taskType} as collected.`);
    }
  };
   ...
   * Includes 'Mark as Collected', 'Delete Task', and 'Edit Task' for Labs and Imaging.
   * @param taskType The type of task ('Lab', 'Imaging', 'Consultation', 'OldConsultation')
   * @param identifier An object containing the fields required to identify the task
   */
  const renderActionButtons = (
    ...
    <View style={styles.actionButtonsContainer}>
      <TouchableOpacity
        ...
        onPress={() => markTaskAsCollected(taskType, identifier)}
      >
  ...
  if (loading) {
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Loading collected tasks...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <SectionList
        ...
        ListEmptyComponent={<Text style={styles.noDataText}>No collected tasks found.</Text>}
